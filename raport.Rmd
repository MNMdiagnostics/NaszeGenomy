...
varmiss %>% filter(F_MISS > 0.005) %>% 
  ...
  theme_classic() + ggtitle('N variants with missing call rate > 0.5%') + xlab('Call rate missing')
...
```{r echo=FALSE}
library(png)
img1_path <- "raport_files/figure-gfm/logafcurve.png"
img1 <- readPNG(img1_path, native = TRUE, info = TRUE)
include_graphics(img1_path)
