...
to_include <- read.table('../input/SAMPLES_TO_INCLUDE_210519.txt',sep='\t')
...
ggplot(roh_chrom_lengths, aes(x=Chromosome, y=cnt)) + 
  geom_boxplot(fill='#48C095') + xlab('Chromosome') + ylab('Count of RoH') +
  theme_minimal()
```
...
# image(t(m[dim(m)[1]:1,]), yaxt='n') + axis(2, at=c(1:22)/22, labels=labels, las=2)
# 
