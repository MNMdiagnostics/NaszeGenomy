...
### ClinVar variants pathogenic & likely pathogenic \n filtered with gnomad3g AF < 0.001
...
levels(consequence$Konsekwencje) <-list(`mutacja w regionie 3'UTR` = '3utr_variant', 
                                        `mutacja w regionie 5'UTR` = '5utr_variant',
                                        `przesunięcie ramki odczytu` = 'frameshift_variant',
                                        `insercja/delecja` = 'inframe_indel',
                                        `mutacja missensowna` = 'missense_variant',
                                        `mutacja niekodująca` = 'nonexonic',
                                        inne = 'other',
                                        `wariant splicingowy`='splicing_variant',
                                        `mutacja w kodonie start/stop` = 'start_stop_variant',
                                        `mutacja cicha` = 'synonymous_variant')
...
cons_plot_pl %>% filter(Konsekwencje != 'inne') %>%
  ...
  geom_area(alpha=0.6 , size=.5, colour="white",position = 'fill') +
    scale_fill_viridis(discrete = T,'Typ mutacji') +
  theme_minimal() +
...
xlab('Przedziały częstotliwość alleli') +
  ylab('Procentowy udział mutacji w danym przedziale') 
