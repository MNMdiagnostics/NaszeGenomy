...
group.colors <- c(Patogenny = "#BC0020", Łagodny ="#48C095", `Łagodny/prawdopodobnie łagodny` = "#27384A",
                  Nieznany = '#B6B6B6', `Prawdopodobnie łagodny` = '#5F5F5F')
...
group.colors <- c(Patogenny = "#BC0020", Łagodny ="#48C095", `Łagodny/prawdopodobnie łagodny` = "#27384A",
                   `Prawdopodobnie łagodny` = '#5F5F5F')

brca %>% filter(sig_pl != 'Nieznany') %>%
  group_by(SYMBOL,sig_pl) %>%
  summarise(n=n()) %>%
  ggplot(aes(y=SYMBOL,x=n,fill=reorder(sig_pl,-n))) +
  geom_bar(stat='identity',position='dodge') +
