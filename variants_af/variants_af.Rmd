...
comp_acmg <- acmg %>%
   select(Uploaded_variation,SYMBOL, all_of(af_list), PL_AC) %>%
  mutate(across(AF:gnomAD_NFE_AF,as.character)) %>%
   mutate(across(AF:gnomAD_NFE_AF,as.numeric)) 

kable(comp_acmg %>% select(-Uploaded_variation,-PL_AC,-SYMBOL) %>%
...
write.table(comp_acmg,'comp_acmg.tsv',quote = F,col.names = T,sep='\t')
