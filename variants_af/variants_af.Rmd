...
af_perc_plot <- af_plot %>% rename(af=allele_frequency) %>%
...
ggplot() + 
  ...
  scale_color_manual(values = group.colors,name='small variation type') +
  theme_minimal() +
...
```{r, echo=FALSE}
# consequence <- read.table('../input/diseases/consequence_ready.tsv',header=T,sep='\t')
# consequence$Consequence <- gsub(',.*','',consequence$Consequence)
# write.table(consequence,'consequence_list.tsv',quote = F,col.names = T,sep='\t',row.names = F)
# 
# consequence %>% 
...
```{r, echo=FALSE}
# consequence <- read.csv('consequence_list.csv',header=T)
# 
# 
# 
# 
# consequence %>% group_by(group,Consequence)
#   ggplot(aes(fill=Consequence,y=n,x=group)) +
#   geom_bar(position="fill", stat="identity") +
#   theme_classic() +
#   xlab('Allele frequencies') +
#   ylab('% of variants') +
